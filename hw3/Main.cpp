...
void* sfeThread(void* arg) {
    ...
    for(int i = myIndex - 1; i >= 0; i--) {
        fbar = fbar + args->symbols->at(i).prob;
    }
    
    // calculate chars for SFE
    int n = ceil(log2(1/myData.prob)) + 1;
    double t = fbar * 2;
    ...
    {
        pthread_cond_wait(&args->cond, &args->printMutex);
    }
...
int main(int argc, char* argv[]) {
    ...
    for(auto& c : symbols) {
        c.prob = ((double) c.frequency / line.size());
    }
